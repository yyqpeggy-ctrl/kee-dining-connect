...
// Per-store tax multipliers for realistic variation
const storeTaxMultipliers: Record<string, number> = {
  all: 1,
  flagship: 0.38,
  french: 0.22,
  jingan: 0.25,
...
const TaxTab = () => {
  ...
  const multiplier = storeTaxMultipliers[storeId] || 1;

  const income = useMemo(() => computeIncomeStatement(storeId as StoreId), [storeId]);

  const taxCalendar = useMemo(() => [
    { tax: "增值税", taxEn: "VAT", period: "2026年2月", periodEn: "Feb 2026", deadline: "2026-03-15", status: "待申报", amount: Math.round(43400 * multiplier) },
    { tax: "企业所得税", taxEn: "CIT", period: "2026年Q1", periodEn: "Q1 2026", deadline: "2026-04-15", status: "待申报", amount: Math.round(income.netProfit * 0.25) },
    { tax: "个人所得税", taxEn: "IIT", period: "2026年2月", periodEn: "Feb 2026", deadline: "2026-03-15", status: "已申报", amount: Math.round(28500 * multiplier) },
    { tax: "城建税", taxEn: "Urban Maintenance", period: "2026年2月", periodEn: "Feb 2026", deadline: "2026-03-15", status: "已申报", amount: Math.round(3038 * multiplier) },
    { tax: "教育费附加", taxEn: "Education Surcharge", period: "2026年2月", periodEn: "Feb 2026", deadline: "2026-03-15", status: "已申报", amount: Math.round(1302 * multiplier) },
    { tax: "印花税", taxEn: "Stamp Duty", period: "2026年2月", periodEn: "Feb 2026", deadline: "2026-03-15", status: "已缴纳", amount: Math.round(650 * multiplier) },
  ], [multiplier, income.netProfit]);

  const taxHistory = useMemo(() => [
    { period: "2026年1月", periodEn: "Jan 2026", vat: Math.round(38200 * multiplier), income: 0, personal: Math.round(26800 * multiplier), total: Math.round(68560 * multiplier), status: "已完成" },
    { period: "2025年12月", periodEn: "Dec 2025", vat: Math.round(42100 * multiplier), income: 0, personal: Math.round(29200 * multiplier), total: Math.round(75120 * multiplier), status: "已完成" },
    { period: "2025年11月", periodEn: "Nov 2025", vat: Math.round(35600 * multiplier), income: 0, personal: Math.round(25100 * multiplier), total: Math.round(63890 * multiplier), status: "已完成" },
    { period: "2025年Q4", periodEn: "Q4 2025", vat: 0, income: Math.round(138500 * multiplier), personal: 0, total: Math.round(138500 * multiplier), status: "已完成" },
    { period: "2025年10月", periodEn: "Oct 2025", vat: Math.round(39800 * multiplier), income: 0, personal: Math.round(27500 * multiplier), total: Math.round(70650 * multiplier), status: "已完成" },
  ], [multiplier]);

  const invoiceStats = useMemo(() => ({
    issued: { count: Math.round(1256 * multiplier), amount: Math.round(4250000 * multiplier) },
    received: { count: Math.round(892 * multiplier), amount: Math.round(2180000 * multiplier) },
    pending: { count: Math.round(45 * multiplier), amount: Math.round(128000 * multiplier) },
  }), [multiplier]);
  ...
  return (
    ...
    <div className="space-y-6">
      ...
      <div className="text-xs text-muted-foreground bg-muted/30 rounded-lg p-2">
        {isZh
          ? `📊 当前显示: ${isHQ ? "全部门店汇总" : storeName(true)}税务数据，企业所得税按利润表净利润×25%估算。`
          : `📊 Showing: ${isHQ ? "All stores consolidated" : storeName(false)} tax data. CIT estimated at 25% of net profit.`}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="glass-card rounded-xl p-5">
          ...
          <p className="text-2xl font-bold text-success">¥{(Math.round(416000 * multiplier) / 10000).toFixed(1)}万</p>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="glass-card rounded-xl p-5">
          ...
          <p className="text-2xl font-bold">{income.totalRevenue > 0 ? ((totalTaxDue / income.totalRevenue) * 100).toFixed(1) : "0"}%</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="lg:col-span-2 glass-card rounded-xl p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2">
              <Button variant="outline" size="sm" className="gap-2"><Calendar className="w-4 h-4" />{t("financeMgmt.taxCalendar")}</Button>
              <Button size="sm" className="gap-2"><Upload className="w-4 h-4" />{t("financeMgmt.oneClickFile")}</Button>
            </div>
          </div>
          <Table>
            <TableHeader>
            ...
            <TableBody>
              {taxCalendar.map((item, idx) => (
                <TableRow key={idx}>
                  ...
                  <TableCell>
                    <span className={item.status === "待申报" ? "text-warning" : "text-muted-foreground"}>{item.deadline}</span>
                  </TableCell>
                  <TableCell className="text-right font-medium">¥{item.amount.toLocaleString()}</TableCell>
                  <TableCell>
                    <Badge variant={item.status === "已缴纳" ? "secondary" : item.status === "已申报" ? "outline" : "destructive"} className="text-xs">
                      {item.status === "待申报" ? t("financeMgmt.pendingFiling") : item.status === "已申报" ? t("financeMgmt.filed") : t("financeMgmt.paid")}
                    </Badge>
                  </TableCell>
                  <TableCell>
                    {item.status === "待申报" && <Button variant="ghost" size="sm" className="text-xs h-7 px-2">{t("financeMgmt.file")}</Button>}
                    {item.status === "已申报" && <Button variant="ghost" size="sm" className="text-xs h-7 px-2">{t("financeMgmt.pay")}</Button>}
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }} className="glass-card rounded-xl p-5">
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              <Button variant="outline" size="sm" className="flex-1 gap-2"><Upload className="w-4 h-4" />{t("financeMgmt.uploadInvoice")}</Button>
              <Button variant="outline" size="sm" className="flex-1 gap-2"><Download className="w-4 h-4" />{t("financeMgmt.downloadLedger")}</Button>
            </div>
          </div>
        </motion.div>
      </div>
          ...
          <TableBody>
            {taxHistory.map((item, idx) => (
              <TableRow key={idx}>
                ...
                <TableCell>
                  <Badge variant="secondary" className="text-xs"><CheckCircle className="w-3 h-3 mr-1" />{t("financeMgmt.done")}</Badge>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
      ...
      <TabsContent value="reports"><TaxReportsTab /></TabsContent>
    </Tabs>
  );
};
