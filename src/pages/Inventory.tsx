...
import { Wine, AlertTriangle, TrendingDown, Search, Minus, History, Package, ClipboardCheck } from "lucide-react";
...
import MonthEndStocktake from "@/components/inventory/MonthEndStocktake";
    ...
    <AppLayout>
      ...
      <Tabs defaultValue="inventory" className="w-full">
        <TabsList className="grid w-full grid-cols-3 mb-6 bg-muted/50">
          <TabsTrigger value="inventory" className="gap-1.5 text-xs"><Package className="w-3.5 h-3.5" />{isZh ? "库存总览" : "Inventory"}</TabsTrigger>
          <TabsTrigger value="stocktake" className="gap-1.5 text-xs"><ClipboardCheck className="w-3.5 h-3.5" />{isZh ? "★月末盘点" : "★Stocktake"}</TabsTrigger>
          <TabsTrigger value="logs" className="gap-1.5 text-xs"><History className="w-3.5 h-3.5" />{isZh ? "扣减记录" : "Deduction Log"}</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="logs">
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass-card rounded-xl overflow-hidden">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/30">
                  <th className="text-left py-3 px-4 text-xs text-muted-foreground">{isZh ? "物料" : "Item"}</th>
                  <th className="text-right py-3 px-4 text-xs text-muted-foreground">{isZh ? "数量" : "Qty"}</th>
                  <th className="text-left py-3 px-4 text-xs text-muted-foreground">{isZh ? "原因" : "Reason"}</th>
                  <th className="text-left py-3 px-4 text-xs text-muted-foreground">{isZh ? "关联菜品" : "Menu Item"}</th>
                  <th className="text-left py-3 px-4 text-xs text-muted-foreground">{isZh ? "时间" : "Time"}</th>
                </tr>
              </thead>
              <tbody>
                {logs.map((log) => (
                  <tr key={log.id} className="border-b border-border/50">
                    <td className="py-2.5 px-4">{log.inventory_item_name}</td>
                    <td className="py-2.5 px-4 text-right">-{Number(log.quantity)} {log.unit}</td>
                    <td className="py-2.5 px-4">
                      <Badge variant="secondary" className="text-xs">
                        {log.reason === "order" ? (isZh ? "订单" : "Order") : log.reason === "manual" ? (isZh ? "手动" : "Manual") : log.reason}
                      </Badge>
                    </td>
                    <td className="py-2.5 px-4 text-muted-foreground">{log.menu_item_name || "-"}</td>
                    <td className="py-2.5 px-4 text-muted-foreground text-xs">{new Date(log.created_at).toLocaleString()}</td>
                  </tr>
                ))}
                {logs.length === 0 && (
